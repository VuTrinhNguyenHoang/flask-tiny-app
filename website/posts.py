...
@login_required
def manage_posts():
    return render_template('manage_posts.html', user=current_user)
...
def delete_posts():
    ...
    if post_ids:
        Post.query.filter(Post.id.in_(post_ids), Post.user_id == current_user.id) \
...
def get_posts():
    ...
    if get_all:
        pagination = Post.query.order_by(Post.updated_at.desc()) \
